...
export function useSession(sessionId) {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(sessionRef, (snap) => {
      if (snap.exists()) {
        const data = snap.val();
        setSession(data);
        setSlides(data.slides ? Object.entries(data.slides)
          .map(([k, v]) => ({ id: k, ...v }))
  ...
  const uploadSlide = useCallback(async (file, order) => {
    ...
    const slideRef = ref(db, `sessions/${sessionId}/slides`);
    await push(slideRef, { url, name: file.name, order, uploadedAt: serverTimestamp() });
    return url;
  }, [sessionId]);

  const saveAnnotation = useCallback(async (slideId, dataUrl) => {
    const annotRef = ref(db, `sessions/${sessionId}/annotations/${slideId}`);
    await set(annotRef, { dataUrl, updatedAt: serverTimestamp() });
  }, [sessionId]);
